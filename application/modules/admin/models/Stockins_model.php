...
class Stockins_model extends CI_Model
{
    public $_table = 'stockins';
    ...
    public function get_all_stockins()
    {
        $this->db->select('stockins.id as stockin_id, products.name as product,  stockins.quantity as quantity, stockins.date as date, stockins.updated_at as updated_at')
            ->from($this->_table)
            ->order_by('stockin_id', 'DESC');
        $this->db->join('products', 'products.id = stockins.product_id');
        $query = $this->db->get();
        if ($query->num_rows() != 0) {
            return $query->result_array();
        } else {
