...
class Assets extends Admin_Base_Controller {
	...
	public function index() {
		redirect('admin/assets/all_assets/');
	}
	...
	public function all_assets() {
		try {
			...
			$crud = $this->getGroceryCRUD('assets', 'Assets', 'Assets', 'Manage All Fixed Assets');

			// data Grid view fields
			$crud->columns('assets.name', 'category_id', 'brand_id', 'updated_at');
			...
			// Required fields
			$crud->required_fields('name', 'category_id', 'supplier_id');
			...
			$output = $crud->render();
			$this->load->view('admin/assets/v_assets', (array) $output);
